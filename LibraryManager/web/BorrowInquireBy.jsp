...
function borrowBook(code){
	...
	form1.target="_blank";
	form1.action="write_borrowform.jsp";
...
function returnBook(){
	...
	window.open ('write_returnform.jsp','newwindow1','height=700,width=1000,top=200,left=400,toolbar=no,menubar=no,scrollbars=yes, resizable=no,location=no, status=no') ;
...
<body class="eva_body">
	...
	list = (ArrayList<Book>) request.getAttribute("list_by");
	String name=(String) request.getSession().getAttribute("Bname2");
	%>
<%
	//判断通过条件查询图书是否成功
		String str1 = (String) request.getSession().getAttribute("Inqbookby");
		if (str1 == "1") {
	%>
	<script>
		alert("查询失败！稍后再试！");
	...
	<%
		request.getSession().setAttribute("Inqbookby", "0");
		}
	%>
