...
<body class="eva_body">
	...
	list = (ArrayList<Libcard>) request.getAttribute("libcard");

	String name=(String)request.getSession().getAttribute("byname");
	String card=(String)request.getSession().getAttribute("bylibcardno");
	...
	<%
		//判断通过姓名查询借书证是否成功
		String str = (String)request.getSession().getAttribute("STR_12");
		if (str == "查询失败，稍后再试！") {
	%>
	<script>
		alert(str);
	...
	<div class="center_eva">
		...
		<div class="position_bg" id="position_bg"
			...
			<span class="pngFix"></span>首页> 图书馆信息管理> 借书证管理> 借书证查询
		</div>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0"  align="center">
									...
									<tr>
                                        <td colspan="2" align="center">
                                         <strong>借书证号：</strong>
										<input type="text" name="bylibcardno" value="<%=card%>"/>&nbsp;&nbsp;&nbsp;                                                                                                        
                                        
                                        <strong>姓名：</strong>
										<input type="text" name="byname" value="<%=name%>"/>&nbsp;&nbsp;&nbsp;	
								...
								<div class="info_table2" style="display: balck" id="jiben">
									<div class="info_mar">
										...
										<table id="tab_cek" width="100%" border="0" cellspacing="0"
											...
											<%if(list.size()==0){%>
											<tr datarow="datarow"><td colspan="11">没有符合查询条件的结果！</td></tr>
											<%}else{
...
<div class="page">
    ...
    <%if (curPage > 1){
    %>
    <a href="#" onclick="document.searchForm.action='LibcardManage?page=1&op=searchcard';document.searchForm.submit();">首页</a>
    <a href="#" onclick="document.searchForm.action='LibcardManage?page=<%=curPage - 1%>&op=searchcard';document.searchForm.submit();">上一页</a>
    ...
    <%if (curPage < maxPage){
    %>  
    <a href="#" onclick="document.searchForm.action='LibcardManage?page=<%=curPage + 1%>&op=searchcard';document.searchForm.submit();">下一页</a>
    <a href="#" onclick="document.searchForm.action='LibcardManage?page=<%=maxPage %>&op=searchcard';document.searchForm.submit();">页尾</a>
    <%
    }else {
    %>
    下一页 尾页
    <%
        ...
        <form name="form1" action="LibcardManage?op=searchcard" method="post">
      <input type="hidden" name="bylibcardno" value="<%=card%>"/>
      <input type="hidden" name="byname" value="<%=name%>"/>
