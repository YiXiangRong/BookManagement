...
<script language="javascript">
    ...
    function increase(){
    	...
    	window.open ('increaseUser.jsp','newwindow1','height=700,width=1000,top=200,left=400,toolbar=no,menubar=no,scrollbars=yes, resizable=no,location=no, status=no') ;
...
<body class="eva_body">
	<%
		...
		list = (ArrayList<User>) request.getAttribute("list_user");
	...
	<%
		//判断查询所有用户是否成功
		String str = (String) request.getSession().getAttribute("STR_4");
		if (str == "查询失败，稍后再试！") {
	%>
	<script>
		alert(str);
	...
	<%
		request.getSession().setAttribute("STR_4", "0");
		}
	%>
	<div class="center_eva">
		...
		<div class="position_bg" id="position_bg"
			...
			<span class="pngFix"></span>首页> 图书馆信息管理> 用户管理
		</div>
